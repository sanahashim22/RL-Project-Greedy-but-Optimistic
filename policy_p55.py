# policy_p55.py
# Risk-aware + turn-planning policy for your Ludo environment.

STARTING = -1
DESTINATION = 56
SAFE_SQUARES = [0, 8, 13, 21, 25, 26, 34, 39, 47, 51, 52, 53, 54, 55, 56]

# Probability that the opponent's next (non-empty) roll list contains a die == d,
# derived from your Dice.roll() process in ludo.py:
P_HAS_D = {1: 43/216, 2: 43/216, 3: 43/216, 4: 43/216, 5: 43/216, 6: 35/216}

def _convert_for_opponent(pos: int) -> int:
    # Mirrors Goti.convert_into_opponent_position
...
def _prob_capture_next_turn(newpos: int, opp_positions: list[int]) -> float:
    # If our landing is safe or non-convertible, no capture risk.
    if _is_safe(newpos):
        return 0.0
    ...
    # Combine capture chances from all opponent gotis that are 1..6 behind opp_view
    probs = []
...
class Policy_P55:
    """
    Chooses the first move that maximizes value over the whole turn:
      + win now, capture, enter on 6, land on/stay on safe squares, progress, home-path bonus
      – expected capture risk on opponent's next turn (probabilistic, from Dice.roll()).
    """
    ...
    def __init__(self, weights=None):
        ...
        self.w = {
            ...
            "leave_safe": -9.0,
            "progress": 0.55,
            "risk_prob": -65.0,   # multiply by P(captured next turn)
        }
        if weights:
            self.w.update(weights)

    # ---- one-step apply with reward -------------------------------------------------
    def _step_reward_and_apply(self, me_pos, opp_pos, gi, dice):
        ...
        # Capture now (extra turn handled by env later; we boost it)
        ...
        # Home-path bonus and progress
        if 51 <= newpos <= 56:
            score += self.w["home_path"]
        base = 0 if cur == STARTING else cur
        score += self.w["progress"] * (newpos - base)

        # Risk of being captured on the NEXT opponent turn (probabilistic)
    ...
    # ---- plan across the whole current turn's dice ---------------------------------
